...
def submerge(eq):
    def conv(eq):
        if eq.name[:3] == "d_-"and eq.name != "d_-1":
            return TreeNode("f_mul", [tree_form(eq.name.replace("-", "")), tree_form("d_-1")])
        return TreeNode(eq.name, [conv(child) for child in eq.children])
    eq = flatten_tree(conv(eq))
    def conv2(eq):
        if eq.name == "f_add":
            negterm = []
            for j in range(len(eq.children)-1,-1,-1):
                if eq.children[j].name == "f_mul" and tree_form("d_-1") in eq.children[j].children:
                    for i in range(len(eq.children[j].children)-1,-1,-1):
                        if eq.children[j].children[i] == tree_form("d_-1"):
                            eq.children[j].children.pop(i)
                    if len(eq.children[j].children) == 1:
                        eq.children[j] = eq.children[j].children[0]
                    if len(eq.children[j].children) == 0:
                        eq.children[j] = tree_form("d_1")
                    negterm.append(eq.children.pop(j))
            negterm = [solve(item) if "f_neg" not in str_form(item) else item for item in negterm]
            eq.children += [item.fx("neg") for item in negterm]
...
def remove_duplicates(data_list, compare_func):
    if not data_list:
        return []
    unique_list = [data_list[0]]
    for item in data_list[1:]:
        is_duplicate = False
        for unique_item in unique_list:
            if compare_func(item, unique_item):
                is_duplicate = True
                break
        if not is_duplicate:
...
def integratex(equation, depth, sse_log, var="v_0"):
    ...
    eq4 = solve(dowhile(equation, expand2))
    lst = [eq3, eq2, eq4]
...
def mat0(eq):
    ...
    out = linear(copy.deepcopy(eqlist))
    def rms(eq):
...
def diffsolve_sep2(eq):
    ...
    if eq.name == "f_add" and all(len(varlist(child))==1 and [str_form(x) for x in factorgen(copy.deepcopy(child))].count(str_form(tree_form(varlist(child)[0]).fx("dif")))==1 for child in eq.children):
        for child in eq.children:
            ...
            child = solve(child)
            s.append(copy.deepcopy(integratex(child, v)))
            if s[-1] is None:
                return None
        s.append(allocvar())
    else:
